...
layui.define(["layer"], function (exprots) {
    ...
    var okUtils = {
         ...
         */
        ajax: function (url, type, params, load, async1) {
			// console.log(url, type, params, load, async1);
            ...
            $.ajax({
				...
				beforeSend: function() {
					if($('.load0').length==0  ){
						$("body").prepend(
											`<span class="load0 layui-icon layui-icon-loading layui-anim layui-anim-rotate layui-anim-loop" style="   position: absolute;z-index: 10;font-size: 3rem;
						color: #656565;
						left: 49%;
						top: 48%;"></span>`
										);
                ...
                success: function (data) { 
					$('.load0').remove();
					// console.log('success');
					try { if (res.status==401||res.statusCode==401||res.data.statusCode==401||res.status==434||res.statusCode==434||res.data.statusCode==434) parent.log_off(); } catch {}
        ...
        tableBatchCheck: function (table,) {
            ...
            if (rows > 0) {
                ...
                for (var i = 0; i < checkStatus.data.length; i++) {
                    ...
                    idsStr += checkStatus.data[i].GUID + ",";
                }
                return idsStr;
            } else {
