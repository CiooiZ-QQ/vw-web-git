...
var config;
 ...
 if (localStorage.getItem(g_accr_login_info) != null) {
	...
	var nick_name=login_information.nick_name,usid=login_information.usid;
 };
...
layui.define(["element", "jquery", "okUtils"], function (exports) {
    ...
    */
   function createMenu(data, tabID, menutitle='') {
      ...
      if (data.children != undefined && data.children.length > 0) {
		 ...
		 temp += '<cite name="'+data.title+'" data-i18n-text="Web22-' + data.title+'" data-parent-name="'+menutitle+'">' + data.title + '</cite>';
         temp += '<span class="layui-nav-more"></span>';
         temp += '</a>';
         temp += '<dl class="layui-nav-child">';
         for (var i = 0; i < data.children.length; i++) {
			if (usid!='5660f012-1351-42f8-955a-e470325e0847'&&data.children[i].title.toLowerCase()=="menu management"  ) {
				var is_tab=false;
			}else{
				var is_tab=true; 
			};
			if(is_tab){
				...
				createMenu(data.children[i], lay_id, data.title);
				temp += '</dd>';
			};
         }
         temp += "</dl>";
		 ...
		 if (usid!='5660f012-1351-42f8-955a-e470325e0847'&&data.title.toLowerCase()=="menu management"  ) {
		 	var is_tab=false;
		 }else{
		 	var is_tab=true; 
		 };
		 if(is_tab) temp += ('<cite name="{0}" data-i18n-text="Web22-{0}" data-parent-name="{1}">{0}</cite></a>').format(data.title,menutitle);
   ...
   okTab.prototype.positionTab = function (contEle, ele) {
      ...
      for (let i = 0; i < menuSet.length; i++) {
         if ($(menuSet[i]).attr('lay-id') == thatLayId) {
			 if (oldmenuitem) oldmenuitem.css("color", "#BDBDC0");
			...
			$(menuSet[i]).css("color", "#FFFFFF");
			oldmenuitem = $(menuSet[i]);
            break;
         }
      }
   };
   ...
   okTab.prototype.tab = function (e) {
      ...
      element.on("tab({0})".format(filter), function (data) {
		 ...
		 // console.log(thatElem.css("color"));
		 // thatElem.css("color","white");
      });
   };
   ...
   okTab.prototype.tabAdd = function (_thisa) {
      ...
      if (_this.attr("target") == "_blank") {
         ...
         var contentIframe = ("<iframe src='{0}' lay-id='{1}'" +
			...
			"</iframe>").format(url + "?v=" + randomData(10000), tabId); //添加随机数参数是为了上传新代码后刷新页面
   ...
   okTab.prototype.render = function (callback) {
	  ...
	  //console.log(111,_this.tabConfig.url);
      if (_this.tabConfig.url) {
		  var result = localStorage.getItem(g_accr_auth_token);
		  var headers={Authorization:'Bearer '+result};
		$.ajax(_this.tabConfig.url, {
		  	type: 'get', //HTTP请求类型 
		  	data: '',
		  	// dataType: 'json',
    ...
    * 关闭计算CV相关功能tab标签页操作
    */
   okTab.prototype.tabClose_cv = function () {
	   var that = this;
	   let openTabs = $('.ok-tab-title > li strong[lay-id]').not('strong[is-close=false]');
	   var filter = that.tabConfig.tabFilter;
	   openTabs.each(function (a, b) {
		  var menuname = $(b.parentElement).find("cite").attr('data-i18n-text').replace('Web22-', '');
		   if(['Price Index', 'Model Price Index', 'Equipment Gradient', 'Custom New Car'].indexOf(menuname) != -1){
			    element.tabDelete(filter, $(b.parentElement).attr("lay-id"));
		   }
	   })
   };
