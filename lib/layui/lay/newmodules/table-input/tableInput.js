/**
 * @Title：tableInput搜索框提示插件||输入框提示插件
 * @Version：1.0.1
 * @Auth：CiooiZ
 * @Date: 2020/11/3
 * @Time: 10:48
...
layui.define(['jquery', 'table'], function(exports) {
	...
	/**
	 * tableInput搜索框提示插件||输入框提示插件初始化
	 */
	tableInput.prototype.render = function(opt) {
        opt.urlBak=opt.url;
		//设置默认初始化参数
		opt.type = opt.type || 'sug'; //默认sug，传入sug||sugTable
		opt.elem = "#tableInput_" + opt.id;
	...
	function getSugTable(opt) {
		...
		//下拉表格初始化table容器
		let html = '<table id="tableInput_' + opt.obj.getAttribute("id") + '" lay-filter="tableInput_' + opt.obj.getAttribute(
				"id") +
			'"></table>';
		...
		table.on('rowDouble(' + "tableInput_" + opt.id + ')', function(obj) {
			//获取选中行所传入字段的值
