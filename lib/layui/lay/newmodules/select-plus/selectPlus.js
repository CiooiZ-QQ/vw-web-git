...
layui.define('form', function (exports) {
  var $ = layui.$,
    ...
    thisIns = function () {
      ...
      return {
		...
		getCheckedArray: function (attrname) {
		  return that.getCheckedArray.call(that, attrname);
		},
        // 配置数据
        config: options
      }
  ...
  // 获取选中数据
  Class.prototype.getCheckedArray = function (attrname) {
	// console.log(attrname);
	var arr = [];
    var that = this,
      options = that.config,
      checkedName = options.config.checkedName,
      data = options.data.forEach(function(item, index){
        if (item[checkedName] === true) {
