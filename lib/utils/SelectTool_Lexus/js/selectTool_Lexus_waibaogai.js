...
var initSelector = function (selecttype, setdata, doctcontainer) {
	...
	$('#selector_ok').click(function () {
		$(".selector_parent").animate({ left: '-100%' }, "slow");
		//console.log('a');
		commitselector();
	});
...
var initSelector_STYLE = function () {
	$('#STYLE').show();
	$('#selector_STYLE').show();
	var urlstring = (g_setdata.datatype.toUpperCase() == 'PV' ? 'Common/GetPVStyleList' : 'Common/GetStyleList');
	ajax(urlstring, {}, function(result) {
		if (!result) return;
      	//console.log(JSON.stringify(result));
      	
      	var doc = $('#selector_STYLE');
    	var appendhtml = '<div><ul class="selector-tab" style="padding-left: ' + (g_setdata.multiselect ? '55' : '30') + 'px;">'; 
...
var initSelector_SUB_SEG = function () {
	...
	var urlstring = (g_setdata.datatype.toUpperCase() == 'PV' ? 'Common/getPVsubseglist' : 'Common/GetSubSeg');
	ajax(urlstring, {}, function(result) {
		if (!result) return;
      	//console.log(JSON.stringify(result));
      	
      	var doc = $('#selector_SUB_SEG');
    	var appendhtml = '<div><ul class="selector-tab" class="selector-tab" style="padding-left: ' + (g_setdata.multiselect ? '55' : '30') + 'px;">'; 
    	for (var i = 0, j = result.length; i < j; i++) {
...
var initSelector_FUEL = function () {
	$('#FUEL').show();
	$('#selector_FUEL').show();
	var urlstring = (g_setdata.datatype.toUpperCase() == 'PV' ? 'Common/GetPVFueltypeList' : 'Common/GetFueltypeList');
	ajax(urlstring, {}, function(result) {
		if (!result) return;
      	//console.log(JSON.stringify(result));
...
var initSelector_BRD = function () {
	...
	$('#selector_BRD').show();
	$('#divletter').hide();
	var urlstring = (g_setdata.datatype.toUpperCase() == 'PV' ? 'Common/getPVBrdlist' : 'Common/getBrdlist');
	ajax(urlstring, {}, function(result) {
		if (!result) return;
      	// console.log(JSON.stringify(result));
    	...
    	for (var i = 0, j = letterlist.length; i < j; i++) {
        	...
        	appendhtml += '<div><ul class="selector-tab" style="padding-left: ' + (isshowcheckbox ? '55' : '30') + 'px;">';
...
var initSelector_SUB_CAR = function (id, name) {
	...
	ajax('Common/GetSubCarList', {
    	...
    	for (var i = 0, j = result.length; i < j; i++) {
        	...
        	appendhtml += '<div><ul class="selector-tab" style="padding-left: ' + (isshowcheckbox ? '55' : '35') + 'px;">';
...
var initSelector_TYPE = function (id, name) {
	...
	$('#selector_TYPE').show();
	$('#selector_VSN').hide();
	ajax('Common/GetTypeListwithsubcar', {
		subcarid: id
	}, function(result) {
		if (!result) return;
      	//console.log(JSON.stringify(result));
    	...
    	for (var i = 0, j = result.length; i < j; i++) {
        	...
        	appendhtml += '<div><ul class="selector-tab" style="padding-left: ' + (g_setdata.multiselect ? '55' : '35') + 'px;">';
        	for (var k = 0, m = result[i].list.length; k < m; k++) {
...
var initSelector_VSN = function (id, name) {
	...
	ajax('Common/GetVsnList', {
    	...
    	doc.html('<h3 style="position: relative;" data-id="' + result.SUB_CAR_ID + '">'
			...
			+ '<div><ul class="selector-tab" id="vsnlist" style="position: relative; padding-left: ' + (g_setdata.multiselect ? '55' : '35') + 'px;"></ul></div>');
...
var ajax = function (urlstring, data, success) {
	$.ajax({
        ...
        beforeSend: function (XMLHttpRequest) {
            XMLHttpRequest.setRequestHeader("userid", g_setdata.userid);
            XMLHttpRequest.setRequestHeader("sessionkey", g_setdata.sessionkey);
        },
