...
var objOkTab = "";
layui.use(["element", "form", "layer", "okUtils", "okTab", "okLayer", "okContextMenu", "okHoliday", "laydate"],
	function() {
		...
		var roleid = (localStorage.getItem(g_accr_login_info) != null) ? JSON.parse(localStorage.getItem(
			g_accr_login_info)).role_id : ""; //获取当前登录用户的角色ID

		/**
		 * 左边菜单显隐功能
		 */
		$("#ok-menu").click(function() {
			$('#tabContent .layui-tab-item').hide();
			$('#tabContent .layui-tab-item.layui-show').show();
			// $(".layui-layout-admin").toggleClass("ok-left-hide");
			// $('.user-photo').toggleClass("ok-menu-hide");
			$("#index_content").toggleClass("ok-left-hide");
			$("#EditionRegion span").toggleClass("hide");
		...
		//console.log(10001,headers);

		var okTab = layui.okTab({
			// 菜单请求路径
			// url: "data/navs.json",
			url: g_req_url + "RoleMenuAuth/GetMenuByRoleID?userid=" + localStorage.getItem(g_accr_auth_usid),
			//headers,
			// 允许同时选项卡的个数
			openTabNum: 30,
			// 如果返回的结果和navs.json中的数据结构一致可省略这个方法
			parseData: function(data) {
				if (!Array.isArray(data)) { var data = JSON.parse(data).Data; };
				console.log('左侧列表数据', (data));
				// var arr_json1 = {
				// 	"title": "营销分析系统",
				// 	"href": "",
				// 	"fontFamily": "ok-icon",
				...
				// 	"children": [{
				// 		"title": "经销商查询",
				// 		"href": "pages/marketing_admin/dealer_query.html",
				...
				// };
				//data.push(arr_json1);
				okUtils.local(g_accr_auth_menus, data);
				return data;
				// okUtils.ajax(g_req_url + "RoleMenuAuth/GetMenuByRoleID", "get", { roleid: 1 }, true).done(function (data) {
				// 	// console.log("success");
				// 	console.log(data);
		...
		objOkTab = okTab;
		// console.log("okTab:", JSON.stringify(okTab));
		...
		$("body").on("click", "#navBar .layui-nav-item a, #userInfo a", function() {
			console.log('切换项目左侧菜单栏时', $("#navBar .layui-this cite").text());
			// if($('#tabContent .layui-show iframe').contents().find('.load0').length>0 ){ 
			//  layer.msg('The page is loading, please try again later!');
			//  return;
			// };
			// 如果不存在子级
			...
			if (!$(this).attr("lay-id")) {
				...
				var childs = $("#navBar > li > dl.layui-nav-child").not(topLevelEle.children(
					"dl.layui-nav-child"));
		...
		$("body").on("click", ".layui-layout-admin .ok-left a[data-url], .ok-make", function() {
			var parent_name = $(this).find('cite').attr('data-parent-name');
			Print_Log(parent_name == '' || parent_name == undefined ? $(this).find('cite').text() :
				parent_name, $(this).find('cite').text(), "Loading");
			if ($(window).width() < 767) {
				$('#index_content').removeClass('ok-left-hide');
			};
		 ...
		 * 关闭所有tab页 - 切换语音版本时
		 */
		$("body").on("click", "#tabAll a", function() {
			okTab.tabClose(3);
		});
		...
		$("#user-info").click(function() {
			okLayer.open("基本资料", "pages/member/user-info.html", "50%", "60%", function(layero) {},
			function() {});
		});
		...
		$("#user-pwd").click(function() {
			okLayer.open("修改密码", "pages/member/user-pwd.html", "29%", "34%", function(layero) {},
		function() {});
		});
		/**
		 * 弹窗皮肤
			...
			okLayer.confirm("确定要退出吗？", function(index) {
				okTab.removeTabStorage(function(res) {
					...
					if (localStorage.getItem(g_accr_login_info) != null) {
						...
						login_information.role_id = "";
						login_information.nick_name = "";
						login_information.usid = "";
						login_information.password = "";
						window.localStorage.setItem(g_accr_login_info, JSON.stringify(login_information))
					}
					window.location.replace(g_req_login_url);
				});
			});
		});
